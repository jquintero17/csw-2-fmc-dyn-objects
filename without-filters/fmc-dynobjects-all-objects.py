    ...
    if positive_diff_names:
        ...
        for new_name in positive_diff_names:
            ...
            if file_obj:
                ...
                timestamped_print(f"Checking if '{sanitized_name }' already exists in API names: {api_names}")
                if sanitized_name not in api_names:  # Ensure no naming conflict
                    timestamped_print(f"Adding new dynamic object: {sanitized_name}")
                    add_payload.append({
                        "name": sanitized_name,  # sanitized named
                        "type": "DynamicObject",         
                        "objectType": "IP",              
                        # Add any additional required fields here if needed
                    })
                else:
                    timestamped_print(f"Skipping addition: {sanitized_name} already exists in the system.")
        ...
        if add_payload:  # Check if there are any objects to add
            ...
            try:
                ...
                if response is not None:
                    timestamped_print(f"Response Status Code: {response.status_code}")
                    timestamped_print(f"Response Content: {response.content.decode()}")
                    if 'Naming Conflict' in response.content.decode():
                        timestamped_print(f"Resolved Conflict: The name already exists, consider updating the existing object instead.")
            except requests.exceptions.RequestException as err:
...
if __name__ == "__main__":
    ...
    prefix_filter = "csw-fmc-"  # Use hardcoded prefix instead of input from argument
    ...
    while True:
        ...
        try:
            ...
            # Filter objects based on hardcoded prefix "csw-fmc-"
            ...
            # Save the filtered objects to a JSON file
            save_json_to_file('filtered_dynamic_objects.json', filtered_objects)
